...
const createWindow = () => {
  ...
  const mainWindow = new electron__WEBPACK_IMPORTED_MODULE_0__.BrowserWindow({
    ...
    height: 600,
    webPreferences: {
      preload: '/mnt/data/git/rsf_updater/.webpack/renderer/main_window/preload.js'
    }
  });
...
electron__WEBPACK_IMPORTED_MODULE_0__.ipcMain.on("notify", (_, text) => {
  new electron__WEBPACK_IMPORTED_MODULE_0__.Notification({
    title: "Notification",
    body: text
  }).show();
});
electron__WEBPACK_IMPORTED_MODULE_0__.ipcMain.on("notifyb", (_, message) => {
  new electron__WEBPACK_IMPORTED_MODULE_0__.Notification({
    title: "Notification",
    body: message
  }).show();
});
electron__WEBPACK_IMPORTED_MODULE_0__.ipcMain.on("savetext", (_, text, fileName) => {
  let targetPath = __dirname;
  if (node_path__WEBPACK_IMPORTED_MODULE_1___default().basename(targetPath) == 'main') {
    //This block of code should only matter during development.
    targetPath = node_path__WEBPACK_IMPORTED_MODULE_1___default().dirname(targetPath);
    if (node_path__WEBPACK_IMPORTED_MODULE_1___default().basename(targetPath) == ".webpack") {
      targetPath = node_path__WEBPACK_IMPORTED_MODULE_1___default().dirname(targetPath);
    }
  }
  new electron__WEBPACK_IMPORTED_MODULE_0__.Notification({
  ...
  fs__WEBPACK_IMPORTED_MODULE_3___default().writeFile(node_path__WEBPACK_IMPORTED_MODULE_1___default().join(targetPath, fileName).toString(), text, function (err) {
    if (err) {
      new electron__WEBPACK_IMPORTED_MODULE_0__.Notification({
        title: "Error",
        body: "Failed to write file: " + err
      }).show();
    }
  });
});
